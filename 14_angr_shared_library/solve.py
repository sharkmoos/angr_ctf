...
def main(argv):
  ...
  # (!)
  buffer_pointer = claripy.BVV(0x3000000, 32) # any empty area of memory
  bufferSize = claripy.BVV(8, 32) # password is 8 chars
  ...
  initial_state.memory.store(buffer_pointer , password) # stash the password where we can access it
  ...
  # constrain eax
  # (!)
  check_constraint_address = base + 0x782
